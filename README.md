...
Есть пример esp8266 в нем не меняются числа, но мигают точки. По скольку библиотека заточена под avr таймер1 который esp8266 не работает. Если хотите сделать часы с wi-fi, то нужно еще разобратся Ticker.h иначе часы будут обновлятся как им вздумается. Но с датчиками такого нет.
